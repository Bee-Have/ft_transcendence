...
function BallRoutine({
    ...
    const handleCollision = () => {
		...
		if (ballRect === null || playerRect === null || opponentRect === null) return;
        ...
        if (ballRect?.left <= 0) {
            ...
            setPosition((prevState) => ({
                x: prevState.x,
                // RetroPongBall.css sets position in %, '100' represents the bottom of window.innerWidth
                y: 100 - ballRadius,
            }));
            ...
            Velocity.current =
                Velocity.current +
                Velocity.current / (BounceCount.current + 10);
    ...
    const animate: FrameRequestCallback = (time: number) => {
        if (previousTimeRef.current !== 0) {
            ...
            setPosition((prevState) => ({
                ...
                x:
                    prevState.x +
                    Direction.current.x * Velocity.current * deltaTime,
                y:
                    prevState.y +
                    Direction.current.y * Velocity.current * deltaTime,
            }));
        }
